...
//                        "    lastLogin = date('now')"
    ...
    public void onLogin (User user, OnCompleteListener<Boolean> callback) {
        firestore.collection("user").whereEqualTo("userId", user.getUserId()).get().addOnCompleteListener(task -> {
            if (task.isSuccessful() && task.getResult() != null) {
                callback.onComplete(Tasks.forResult(true));
            } else {
                Log.e("User Login", "User is not found: " + user.getUserId());
    ...
    public String getDocumentId(Object entity) {
        if (entity instanceof Identifiable) {
            return ((Identifiable) entity)._getId();
        }
        return null;
    }
    ...
    private <T> void handleFirestoreInsert(String tableName, T entity) {
        if (entity instanceof Identifiable) {
            String docId = ((Identifiable) entity)._getId();
    ...
    private <T> void handleFirestoreUpdate(String tableName, T entity) {
        firestore.collection(tableName).get().addOnSuccessListener(querySnapshot -> {
            String matchingId = ((Identifiable) entity)._getId();
    ...
    private <T> void handleFirestoreDelete(String tableName, T entity){
        try{
            ...
            if (!getIdFieldName(tableName).equals("autogenerated")) {
                firestore.collection(tableName).document(getDocumentId(entity)).delete().addOnSuccessListener(aVoid -> {
...
//    }
    ...
    public <T> void executeAction(String action, String tableName, T entity, Context context) {
        ...
        // Check and handle DELETE first because temp db not needed
